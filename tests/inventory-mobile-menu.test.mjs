...
assert.match(source,/function renderInventoryMobileEntry\(\)[\s\S]*?renderInventoryTabDesktop\(\)[\s\S]*?data-stock-mobile-back="entry"[\s\S]*?Thêm sản phẩm/,'The mobile add-product screen must reuse the real inventory entry form and provide back navigation');
