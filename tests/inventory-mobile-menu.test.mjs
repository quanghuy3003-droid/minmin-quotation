...
assert.match(source,/function addInventoryItemToQuote\(code,qty=1\)[\s\S]*?inventoryQuoteAddLock[\s\S]*?quoteContainsOnlyLegacySamples\(\)[\s\S]*?_inventorySourceKey[\s\S]*?existing\.quantity[\s\S]*?state\.lines\.push\(line\)/,'Adding from mobile inventory must be single-shot, remove legacy samples and merge only the selected product');
assert.match(source,/const quickQuote=event\.target\.closest\?\.\('\[data-stock-mobile-quick-quote\]'\)[\s\S]*?event\.stopImmediatePropagation\(\)[\s\S]*?addInventoryItemToQuote\(quickQuote\.dataset\.stockMobileQuickQuote,1\)/,'The mobile quick-quote action must stop duplicate delegated click handling');
assert.match(source,/function defaultLines\(\)\{\s*return \[\];\s*\}/,'A new quotation must no longer be populated with legacy sample products');
