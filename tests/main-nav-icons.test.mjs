...
assert.match(source,/body \.minmin-main-nav \.minmin-tab-button \{[\s\S]*?width:\s*108px !important;[\s\S]*?min-width:\s*108px !important;[\s\S]*?max-width:\s*108px !important;/,'Desktop tab slots must keep fixed positions when the active tab changes');
assert.match(source,/white-space:\s*nowrap !important;/,'Navigation labels must stay on one line');
