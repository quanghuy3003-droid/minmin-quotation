...
assert.match(headerSource,/✓ Đã đủ liên kết/,'A fully linked order needs a green check state');
assert.match(headerSource,/Còn thiếu \$\{missing\} liên kết/,'An incomplete order needs a visible missing-link count');
assert.match(headerSource,/border-mint\/40 bg-mint\/10/,'Complete orders need a light green background');
assert.match(headerSource,/border-orange-200 bg-orange-50/,'Incomplete orders need a light orange background');
assert.match(headerSource,/Kết quả đơn hàng/);
assert.match(headerSource,/Doanh thu/);
assert.match(headerSource,/Chi phí vào/);
assert.match(headerSource,/Lợi nhuận/);
