...
const headerSource=section('function outGHeader','function outGLineCost');
assert.match(headerSource,/Hóa đơn đầu ra/,'The invoice identity must be the main heading');
assert.match(headerSource,/Ngày hóa đơn/);
assert.match(headerSource,/Ký hiệu/);
assert.match(headerSource,/Tổng sau VAT/);
assert.match(headerSource,/Còn phải thu/);
assert.match(headerSource,/outGMetric\('VAT đầu ra'/);
assert.match(headerSource,/outGMetric\('VAT đầu vào đã khớp'/);
assert.match(headerSource,/outGMetric\('VAT chênh lệch'/);
assert.match(headerSource,/outGMetric\('Lợi nhuận tạm tính'/);
assert.match(html,/active\?outGLinesTable\(active\):''/,'The XML line-matching table must remain visible for the selected invoice');
