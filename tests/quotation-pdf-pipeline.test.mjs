...
assert.match(source,/reservedForClosing=final\?\(24\+20\*7\):0/,'Pagination must reserve space for all seven total rows');
assert.match(source,/\.inline-total-row\{[^}]*margin:0 0 0 auto/,'All totals must sit directly under the right side of the product table');
assert.match(source,/\.sheet th\{[^}]*vertical-align:middle!important/,'Column headings must center vertically within their cells');
assert.match(source,/\.sheet th,\.sheet td\{border:\.25px solid #555/,'Quotation grid lines must render extra thin and soft');
assert.match(source,/\.inline-total-row\{[^}]*border:\.25px solid #555/,'Total grid borders must match the extra-thin product grid');
